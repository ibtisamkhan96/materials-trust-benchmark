...
def build_readme() -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "The UI in `site/` is the public face of the run: coverage, the "
            "headline disagreements, the figures, a lookup over all 287 assayed "
            "compositions, and a live panel. The live panel does not use a shared "
            "key. The visitor supplies a GitHub token (identity only), a Materials "
            "Project key (the numbers), and an Anthropic key (the explanation). "
            "Keys stay in the browser tab and are posted only to this site's "
            "Netlify functions. Live grouping is by space group, not pymatgen "
            "`StructureMatcher`; the CLI remains the authoritative audit. Rebuild "
            "the compact index after a new benchmark with "
